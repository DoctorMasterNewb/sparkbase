...
| S-sm121-nvfp4 | first-party | **Correction source.** NVIDIA PTX ISA 8.8 (Table 64, family-specific features), CUDA C Programming Guide (Table 28, family compatibility), CUTLASS "Blackwell SM120 GEMMs" (instruction + tile-shape tables) read via the `nvidia-cuda-docs` MCP, cross-checked against a running GB10: symbol scan of the shipped `vllm-node-v0260` `_C`/`_moe_C`/`_qutlass_C` extensions, `fused_moe/oracle/nvfp4.py`, `experts/flashinfer_b12x_moe.py`, `cute/arch/config.hpp`. Establishes that sm_121 HAS native block-scaled FP4/FP8 MMA and that the Marlin fallbacks are kernel-coverage gaps. | `nvidia-cuda-docs` MCP + first-party inspection on `gx10-5f44` | 2026-08-22 |
| S-b12x-ab | first-party | NVFP4 MoE backend A/B on the head GB10: `auto` (FLASHINFER_CUTLASS) vs `flashinfer_b12x` vs `--linear-backend flashinfer_b12x`, 3 verified-distinct runs/arm across 2 profiles, engine kernel-selection lines captured per arm, GPU clock/power sampled throughout. Also the sparkrun benchmark-cache replay discovery and six further harness traps. | first-party runs on `gx10-5f44`; `~/services/b12x-moe-bench/` | 2026-08-22 |
| S-gb10-profile | first-party | GB10 decode-step characterisation: cudagraph-vs-eager A/B (2 interleaved reps), torch-profiler trace of prefill+64 decode tokens on Qwen3.6-35B-A3B-NVFP4 (89k kernel events), checkpoint `ignore`-list audit showing 19% NVFP4 adoption, machine-balance arithmetic, and the GB10 profiling recipe (ncu counter availability on sm_121, `--profiler-config` gating). Method framing from arXiv 2607.14541 (Atrex-Bench / Atrex-Kernel-Agent). | first-party runs on `gx10-5f44`; arxiv.org/html/2607.14541v1 | 2026-08-23 |
| S-dsv4-opt | first-party | DeepSeek-V4-Flash CRACK optimisation pass on a GB10 pair: safetensors dtype audit (MXFP4 experts + MXFP8 attention despite an NVFP4 name), torch-profiler decode trace (67k kernel events: 38% W4A16 / 27.6% native FP8-FP4 / 13.7% SM80 WMMA / 7.3% NCCL), KV-pin A/B (+23% pool), and the warmup / cell-stability / knob-verification methodology findings. | first-party runs on `gx10-5f44` + `gx10-1c23` | 2026-08-23 |
| S-dsv4-vision | first-party | Porting webbrain-one's MoonViT vision adapter for DeepSeek-V4-Flash-0731 (frozen Kimi-K2.6 tower 833 MB + trained PatchMerger projector 80 MB) from its source-patched SGLang extension to the vLLM DSpark stack on a GB10 pair, over dealignai's MXFP4 CRACK weights. Serves end to end (50/50 shards, all 335 vision tensors bound, KV pool 2,272,730 tokens, DSpark k=5 alongside a multimodal target) but the model is functionally blind: solid red/blue/green -> pink/blue/white. Instrumented tower/projector: projector output norm ~127-136 vs token-embedding norm 7.3 (CRACK) / 4.9 (webbrain's own NVFP4 half) = 26x oversized, >98% of each image embedding a constant offset; preprocessing correct (pixels cosine -0.33), tower content-sensitive (red vs noise 41% rel-L2, red vs red 0). Embedding-scale sweep 0.005-0.5, no working setting. Port verified line-by-line against SGLang commit fdebc938. Plus six GB10 traps: ViT flash-attn PTX toolchain rejection, cudagraph replay IMA, BPE placeholder merging, missing jinja chat template, OOV padding-row gather, hash-routing raw-id requirement. Author's own NVFP4 text half unservable (no NvFp4 MoE backend; Marlin repack PTX wall). | first-party runs on `gx10-5f44` + `gx10-1c23`, 2026-08-24 | 2026-08-24 |

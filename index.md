...
- [platform-gb10](wiki/platform-gb10.md) — the hardware: **machine balance ~916 FLOP/byte (~7,300 concurrent tokens to saturate FP4 ⇒ decode tensor-core idleness is arithmetic, not waste)**, sm_121/12.1a, 121 GB unified, ~270 GB/s (decode is bandwidth-bound), **native block-scaled FP4/FP8 MMA (`compute_120f` family) — corrected 2026-08-22, this said the opposite and was tagged `[proven]`**, no GPUDirect, OOM=reboot. **Read first.**
...
- [cudagraphs-and-compile](wiki/cudagraphs-and-compile.md) — the three cudagraph walls (MoE on sm_121, cross-node host-staged NCCL / vllm#46253, grafted-multimodal replay IMA) and the "20 tok/s" math.
...
- [vision-adapters](wiki/vision-adapters.md) — grafting a frozen vision tower + trained projector onto a text-only LLM: **why a wired adapter can still be blind, and the three measurements that catch it**; the embedding-norm ratio test; ViT flash-attn PTX wall (`--mm-encoder-attn-backend TORCH_SDPA`); BPE-merged placeholders; no-jinja-chat-template image drop; OOV padding-row gather; hash-routed MoE needing raw token ids; SGLang→vLLM porting notes.
